...
if [[ "$EXPECTED_FW" > "$CURRENT_FW" ]]; then
	...
	if [[ "$EXPECTED_FW" = "8.10" ]]; then
		...
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v8_10_Linux.tar.gz"
		...
		echo "updating to $EXPECTED_FW version"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v8_60_Linux.tar.gz"
		...
		echo "updating to $EXPECTED_FW version"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v8_70_Linux.tar.gz"
		...
		echo "updating to $EXPECTED_FW version"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v9_00_Linux.tar.gz"
		...
		echo "updating to $EXPECTED_FW version"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v9_20_Linux.tar.gz"
		#scp 700Series_NVMUpdatePackage_v9_00_Linux.tar.gz root@${NODE_ID}:/root/
		...
		echo "updating to $EXPECTED_FW version"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMUpdatePackage_v9_30_Linux.tar.gz"
		#scp 700Series_NVMUpdatePackage_v9_00_Linux.tar.gz root@${NODE_ID}:/root/
...
if [[ "$EXPECTED_FW" < "$CURRENT_FW" ]]; then
	...
	if [[ "$CURRENT_FW" = "8.30" ]]; then
		...
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
                ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMDowngradePackage_v8_10_Linux.tar.gz"
		...
		echo "downloading system firmware to $EXPECTED_FW"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
  		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMDowngradePackage_v8_60_to_v8_10_Linux.tar.gz"
		...
		echo "downloading system firmware to $EXPECTED_FW"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
  		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMDowngradePackage_v8_70_to_v8_10_Linux.tar.gz"
		...
		echo "downloading system firmware to $EXPECTED_FW"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
  		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMDowngradePackage_v9_20_to_v8_10_Linux.tar.gz"
		...
		echo "downloading system firmware to $EXPECTED_FW"
		s3web_url="$(ping -c2 s3web | grep statistics | sed 's/.*s3web/s3web/g' | awk '{ printf $1 }')"
  		ssh root@${NODE_ID} "curl -LO http://${s3web_url}/download/bkc-mirror/wilson_city/network_fw/700Series_NVMDowngradePackage_v9_30_to_v8_10_Linux.tar.gz"
		#scp 700Series_NVMUpdatePackage_v9_20_Linux.tar.gz root@${NODE_ID}:/root/
