...
NIC_NAME=$(ssh root@${NODE_ID} "nmcli d |grep -v 'disconnected' |grep -E ^enp[[:digit:]]s0|grep -i connected|cut -d ' ' -f1")


if [ -z "$NIC_NAME" ]; then
	NICDEV=$(ssh root@${NODE_ID} "nmcli d|grep -v 'disconnected' |grep -E ^enp|grep -i connected|cut -d ' ' -f1")
	NICFW=$(ssh root@${NODE_ID} "ethtool -i $NICDEV|grep -i firmware|cut -d ' ' -f2|cut -c1-4")
else
	NICDEV=$(ssh root@${NODE_ID} "nmcli d|grep -v 'disconnected' |grep -E ^enp|grep -i connected|cut -d ' ' -f1")
	NICFW=$(ssh root@${NODE_ID} "ethtool -i $NICDEV|grep -i firmware|cut -d ' ' -f2|cut -c1-4")
fi	

echo -e "\r$NODE_ID|$ssh_status|$bios_ver|$microcode|$BMC_VER|$CPLD_VER|$NICFW|$Name_image|$kernel" 
