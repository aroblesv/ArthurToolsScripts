...
if [[ "$CosCLV1" = "$EosCLV1" ]]; then
	echo "Columbia1 Done" >> CheckerStatus
else
	echo "Columbia1 Fail" >> CheckerStatus
fi

if [[ "$CosCLV2" = "$EosCLV2" ]]; then
	echo "Columbia2 Done" >> CheckerStatus
else
	echo "Columbia2 Fail" >> CheckerStatus
fi
...
if [[ "$CosArbFw" = "$EosArbFw" ]]; then
	echo "Arbord Done" >> CheckerStatus
else
	echo "Arbord Fail" >> CheckerStatus
fi
...
echo "Verifying Arbord: $EosArbFw"
echo "########################################################################################################################"
...
/^Columbia1/{
  ...
  columbia1Arr[count]=$NF
  next
}
/^Columbia2/{
  if(++count7==1){ header=header OFS $1 }
  columbia2Arr[count]=$NF
  next
}
/^Fortville/{
...
END{
  print header
  for(i=1;i<=count;i++){
  printf("%s %s %s %s %s %s %s %s %s\n",biosArr[i],cpldArr[i],bmcArr[i],kernelArr[i],ucodeArr[i],columbia1Arr[i],columbia2Arr[i],fortvilleArr[i],arbordArr[i])
  }
}
...
grep -i fail ./regCheckerListInfo |awk '{print $10}' > full_list2
